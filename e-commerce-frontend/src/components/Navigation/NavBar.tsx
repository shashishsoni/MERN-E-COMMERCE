      ...
      <nav className="flex items-center justify-between border px-5 py-3 shadow-md">
        <div className="item-center flex gap-x-8">
          ...
          <div className="relative" ref={SearchdropDownRef}>
            ...
            {searchResult?.length > 0 && (
              <div className="absolute mt-2 max-h-[500px] w-full overflow-y-auto bg-slate-50 p-5 shadow-md">
                {searchResult.map((res) => (
                  <div key={res._id} className="flex items-center gap-x-2 pb-5">
                    <img className="mr-9 aspect-square w-16" src={res.image} />
                    <h2 className="text-[14px]">{res.name}</h2>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="flex gap-x-3">
          <NavLink
            to="/Home"
            className={({ isActive }) =>
              `nav-link ${isActive ? 'text-red-700' : ''}`
            }
          >
            Shop
          </NavLink>
